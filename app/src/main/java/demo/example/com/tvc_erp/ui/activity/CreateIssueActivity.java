...
public class CreateIssueActivity extends BaseActivity implements TokenCompleteTextView.TokenListener<String> {

    private ImagesAdapter imageAdapter;
    private ArrayList<Integer> seletedItems = new ArrayList<>();
    private ArrayList<String> listUsers = new ArrayList<>();
    private ArrayList<String> listUserToRemove = new ArrayList<>();
    ArrayAdapter<String> adapter;
    TabHost thCreateIssue;
    private String[] users, types, priorities, buildings, apartments;
    ...
    private void addEvent() {
                ...
                showListUserDialogMultiChoice(users, "Select a user", new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < seletedItems.size(); i++) {
                            ...
                            completionView.addObject(users[postition]);
                            listUserToRemove.add(users[postition]);
                        }
        ...
        edt_start_date.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new DatePickerDialog(CreateIssueActivity.this, R.style.DialogTheme, new DatePickerDialog.OnDateSetListener() {
                    @Override
        ...
        ll_attach.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                pickerHelper = new ImagePickerHelper(CreateIssueActivity.this);
...
//        rvImages.addItemDecoration(new SuggestedPropertiesAdapter.SpacesItemDecoration(20));
    ...
    private void addControl() {
        thCreateIssue = (TabHost) findViewById(R.id.thCreateIssue);
        ...
        //Set text title for 2 tabwidget
    ...
    private void postIssue() {
        ...
        Model.getInstance().postCreateTask(formTask, "I");
        getBusyIndicator(R.string.please_wait).show();
    }
    ...
    //Set up tab screen for left tabwidget
    private void setupTab(final View view, final String tag) {
        thCreateIssue.setup();

        View tabview = createTabView(thCreateIssue.getContext(), tag);
        TabHost.TabSpec setContent = thCreateIssue.newTabSpec(tag).setIndicator(tabview)
                .setContent(new TabHost.TabContentFactory() {
                    public View createTabContent(String tag) {
                        return view;
                    }
                });
        setContent.setContent(R.id.ll_issue_info);
        thCreateIssue.addTab(setContent);
    }

    //Set up tab screen for rigth tabwidget
    private void setupRightTab(final View view, final String tag) {
        thCreateIssue.setup();

        View tabRightView = createRightTabView(thCreateIssue.getContext(), tag);
        TabHost.TabSpec setRightContent = thCreateIssue.newTabSpec(tag).setIndicator(tabRightView)
                .setContent(new TabHost.TabContentFactory() {
                    @Override
                    public View createTabContent(String tag) {
                        return view;
        ...
        setRightContent.setContent(R.id.ll_issue_con);
        thCreateIssue.addTab(setRightContent);
    }

    //Setup screen for left tabwidget
    ...
    //Setup screen for right tabwidget
    ...
    protected void onModelUpdated(ModelEvent evt) {
        ...
        if (evt == ModelEvent.GET_LIST_USERS_SUCCESS) {
            ...
            Toast.makeText(this, "Add issue is successful", Toast.LENGTH_SHORT).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == PICK_IMAGE_LOGO && resultCode == RESULT_OK) {
            ...
            for (Uri uri : uris) {
                photoSize += FileHelper.getSize(CreateIssueActivity.this, uri);
                if (photoSize > GlobalConfig.MAX_FILE_SIZE) {
                    ...
                    break;
                } else if (!FileHelper.checkImageType(CreateIssueActivity.this, uri)) {
    ...
    private String[] removeItemListUser(String nameUser) {
        for (int i = 0; i < users.length; i++) {
            if (users[i].equalsIgnoreCase(nameUser)) {
                adapter.add(nameUser);
            }
        }
        return users;
    }
